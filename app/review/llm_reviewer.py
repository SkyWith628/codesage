...
핵심 설계:
    ...
    - GEMINI_API_KEY가 없으면 mock 리뷰 (키 없이도 시연 가능)
"""
...
async def review(
    ...
    if not settings.llm_enabled:
        logger.warning("GEMINI_API_KEY 없음 → mock 리뷰 반환")
    ...
    try:
        ...
        resp = await llm_client.generate(
            ...
            config=types.GenerateContentConfig(
                # 시스템 프롬프트(역할·루브릭·보안 규칙)를 분리해 주입
                system_instruction=system_prompt,
                max_output_tokens=policy.get("max_tokens", 4096),
                # ★ JSON 출력을 강제 → 코드펜스/잡설 없이 순수 JSON만 반환
                response_mime_type="application/json",
                temperature=0,  # 리뷰는 일관성이 중요 → 무작위성 최소화
            ),
        )
    except APIError as exc:
        # 인증/할당량/레이트리밋 등 API 오류 시 파이프라인을 죽이지 않고
        ...
        if fd.added_lines:
            comments.append(
                ReviewComment(
                    ...
                    body="[MOCK] GEMINI_API_KEY를 설정하면 실제 AI 리뷰가 생성됩니다.",
                    suggested_code=None,
                )
            )
    return ReviewResult(
