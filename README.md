...
| 문제 | 고민 | 해결 |
|---|---|---|
| **Webhook 타임아웃** — PR 리뷰는 LLM 호출이 느려, 동기로 처리하면 GitHub Webhook이 타임아웃 처리 | 수신 응답 속도와 무거운 리뷰 작업을 어떻게 분리하나 | **수신과 처리를 큐로 분리** — Webhook은 서명 검증 후 즉시 200 반환, 실제 리뷰는 Worker가 Redis 큐에서 꺼내 비동기 처리 |
| **토큰 비용·중복 코멘트** — 커밋마다 PR 전체를 재리뷰하면 비용 낭비 + 같은 지적 반복 | 변경분만 정확히 추려낼 수 있나 | **증분 리뷰** — `synchronize` 이벤트에서 새 커밋 diff만 LLM에 투입, 이미 리뷰한 라인은 건너뜀 |
| **신뢰 경계** — Webhook 위조, PR 본문에 섞인 프롬프트 인젝션 | 외부 입력을 어디까지 믿나 | **HMAC 서명 검증** + LLM 프롬프트에서 사용자 입력 격리, **GitHub App 설치 토큰 자동 발급**(JWT→1시간 토큰·캐싱)으로 권한 최소화 |
